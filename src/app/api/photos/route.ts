...
export async function GET(request: NextRequest) {
    try {
        ...
        console.log('[Amateur] Forwarding GET request to upstream API (photos list)');

        const response = await fetch('https://test.bowlersnetwork.com/api/photos', {
            method: 'GET',
            headers,
        });
        ...
        if (!response.ok) {
            ...
            try {
                ...
                errorData = { errors: [`Upstream server returned error ${response.status} on photos list`] };
            }
        ...
        console.error('[Amateur] Fetch photos list error (proxy):', error);
        return NextResponse.json(
            { errors: ['Failed to fetch photos list (network or server error)'] },
            { status: 500 }
        );
    }
}
