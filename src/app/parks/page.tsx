...
export default function ParksPage() {
  ...
  const categories = [
    { id: 'all', name: 'All Partners', icon: Building, count: mockParks.length },
    { id: 'restaurants', name: 'Restaurants', icon: Utensils, count: mockParks.filter(p => p.category === 'restaurants').length },
    { id: 'fitness', name: 'Fitness Centers', icon: Dumbbell, count: mockParks.filter(p => p.category === 'fitness').length },
    { id: 'entertainment', name: 'Entertainment', icon: TreePine, count: mockParks.filter(p => p.category === 'entertainment').length },
    { id: 'shopping', name: 'Shopping', icon: Coffee, count: mockParks.filter(p => p.category === 'shopping').length },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedParks.map((park) => (
            <div key={park.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-gray-500">({park.reviewCount})</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 text-gray-600 mb-2">
                  ...
                  <span className="text-xs bg-gray-100 px-2 py-1 rounded-full">{park.distance}</span>
                </div>

                <div className="flex items-center gap-2 text-gray-600 mb-3">
                  <Clock className="w-4 h-4" />
                  <span className="text-sm">{park.hours}</span>
                </div>

                <p className="text-gray-700 text-sm mb-4">{park.description}</p>
                ...
                {/* Pricing */}
                {park.originalPrice && park.discountedPrice && (
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <span className="text-lg font-bold text-green-600">{park.discountedPrice}</span>
                      <span className="text-sm text-gray-500 line-through ml-2">{park.originalPrice}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-gray-50 rounded-lg p-3 mb-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-mono text-lg font-bold text-green-600">{park.couponCode}</p>
                    </div>
                    <button
                      onClick={() => copyToClipboard(park.couponCode)}
