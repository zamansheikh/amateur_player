...
import { Sparkles, Zap, Loader2, AlertCircle, User, Star, Trophy, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function TradingCardsPage() {
    ...
    const getAuthenticatedUrl = (url: string) => {
        if (!url) return '';
        const token = user?.access_token || (typeof window !== 'undefined' ? localStorage.getItem('access_token') : '');
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-gray-900">Trading Cards</h1>
                                <p className="text-xs text-gray-500">Collect and trade player cards</p>
                            </div>
                        </div>
                        <button
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {loading ? (
                    ...
                    <div className="text-center py-20">
                        ...
                        <p className="text-gray-500 mb-8">Be the first to create a trading card!</p>
                        <button
                            onClick={() => router.push('/trading-cards/generate')}
                            ...
                            {cards.map((card, index) => {
                                ...
                                return (
                                    <div 
                                        ...
                                        style={{
                                            transform: `translateX(${offset * 340}px) scale(${1 - absOffset * 0.15}) perspective(1000px) rotateY(${offset * -5}deg)`,
                                            zIndex: 50 - absOffset,
                                            opacity: absOffset > 2 ? 0 : 1 - absOffset * 0.2,
                                            pointerEvents: isActive ? 'auto' : 'none',
                                            filter: isActive ? 'drop-shadow(0 0 20px rgba(74, 222, 128, 0.3))' : 'none'
                                        }}
                                    ...
                                    >
                                        <div 
                                            ...
                                            onMouseEnter={() => handleCardHover(card.metadata.card_id, card.metadata.user_id)}
                                            onMouseLeave={() => setHoveredCard(null)}
                                        >
                                            {/* Shadow Effect */}
                                            <div className={`absolute -inset-3 rounded-3xl bg-gradient-to-br from-green-400/20 to-blue-500/20 blur-2xl transition-all duration-500 pointer-events-none -z-10 ${
                                            ...
                                            {card.metadata.collections_count > 0 && (
                                                <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm px-3 py-1.5 rounded-full flex items-center gap-1.5 z-10 pointer-events-none">
                                                    ...
                                                    <span className="text-white text-sm font-bold">{card.metadata.collections_count}</span>
                                                </div>
                                            )}
                                            ...
                                            <div className={`absolute inset-0 rounded-2xl transition-all duration-500 ${
                                                ...
                                                <div className={`absolute inset-x-0 bottom-0 h-auto bg-gradient-to-t from-black via-black/90 to-transparent backdrop-blur-md transition-transform duration-500 rounded-b-2xl ${
                                                        ...
                                                        <div className="p-6 w-full">
                                                            ...
                                                            <div className="flex items-center gap-4 mb-4">
                                                                    ...
                                                                    {userMinimal.profile_picture_url ? (
                                                                        ...
                                                                        <div className="w-16 h-16 rounded-full border-3 border-white shadow-lg bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center">
                                                                            <span className="text-2xl font-bold text-white">
                                                                                {userMinimal.first_name?.[0]}{userMinimal.last_name?.[0]}
                                                                            </span>
                                                                        </div>
                                                                    )}
                                                                ...
                                                                <div className="flex-1">
                                                                    ...
                                                                    <p className="text-gray-300 text-sm">@{userMinimal.username}</p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="flex items-center gap-3 mb-4 text-sm">
                                                                <div className="flex items-center gap-1.5 text-white/90">
                                                                    <Star className="w-4 h-4 text-yellow-400" />
                                                                    <span className="font-medium">{card.metadata.collections_count}</span>
                                                                    <span className="text-gray-300">Collections</span>
                                                                </div>
                                                                <div className="flex items-center gap-1.5 text-white/90">
                                                                    <User className="w-4 h-4 text-blue-400" />
                                                                    <span className="font-medium">{userMinimal.follow_info.follwers}</span>
                                                                    <span className="text-gray-300">Followers</span>
                                                                </div>
                                                                <div className="flex items-center gap-1.5 text-white/90">
                                                                    <Zap className="w-4 h-4 text-purple-400" />
                                                                    <span className="font-medium">{userMinimal.xp}</span>
                                                            ...
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    router.push(`/profile/${userMinimal.username}`);
                                                                }}
                                ...
                                {cards.map((_, idx) => (
                                    <button
                                        ...
                                        className={`w-2 h-2 rounded-full transition-all ${
                                            idx === currentIndex ? 'w-6 bg-green-500' : 'bg-gray-300 hover:bg-gray-400'
                                        }`}
                                    />
                                ))}
                            </div>
